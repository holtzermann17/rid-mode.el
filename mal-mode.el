...
(setq abandonment (concat "\\<" (regexp-opt '("feel" "will" "leave" "upset" "losing" "they'll" "they're" "addicted" "afraid" "available" "base" "clinging" "coming" "committed" "defenseless" "desperate" "die" "drive" "expect" "fighting" "last" "leaves" "leaving" "next" "nice" "partners" "period" "prefer" "pulling" "regular" "self-absorbed" "short" "soon" "stable" "unpredictable" "worried")  t) "[A-Za-z]*\\>" "\\|" "\\<" (regexp-opt '("count")  t) "\\>")

      emotional-deprivation  (concat "\\<" (regexp-opt '("haven't" "advice" "warmth" "wanted" "tuned" "true" "spend" "special" "sound" "share" "nurture" "holding" "himself" "herself" "gotten" "felt" "deeply")  t) "[A-Za-z]*\\>" "\\|" "\\<" (regexp-opt '()  t) "\\>")

      emotional-inhibition  (concat "\\<" (regexp-opt '("emotionally" "anger" "warm" "uptight" "unemotional" "spontaneous" "show" "seriously" "positive" "likely" "inside" "gets" "emotions" "embarrassing" "build" "affection" "actions") t) "[A-Za-z]*\\>"  "\\|" "\\<" (regexp-opt '()  t) "\\>")

      enmeshment  (concat "\\<" (regexp-opt '("parent" "parents" "partner" "separate" "each" "living" "intimate" "speak" "separating" "privacy" "overinvolved" "other's" "lives" "keeping" "identity" "distance" "disappointed" "details" "day" "betrayed" "age")  t) "[A-Za-z]*\\>")

      entitlement  (concat "\\<" (regexp-opt '("shouldn't" "tolerate" "tell" "rules" "restrictions" "priorities" "placed" "offer" "normal" "many" "kept" "irritated" "irritable" "hate" "friends" "follow" "conventions" "controlling" "contributions" "constrained" "answer" "ahead")  t) "[A-Za-z]*\\>")

      insufficient-self-control  (concat "\\<" (regexp-opt '("goal" "easily" "complete" "temper" "stop" "stick" "start" "smoking" "slightest" "routine" "resolutions" "regret" "reach" "problem" "persevere" "overeating" "once" "offense" "long-range" "later" "impulsively" "hold" "frustrated" "force" "enjoy" "drinking" "discipline" "cost" "concentrate" "bored" "behaviors" "anything" "allow" "self-control" "self-discipline")  t) "[A-Za-z]*\\>" "\\|" "\\<" (regexp-opt '()  t) "\\>")

      mistrust-abuse  (concat "\\<" (regexp-opt '("motives" "before" "advantage" "throughout" "well-intentioned" "used" "truth" "trusting" "trust" "towards" "suspicious" "purposes" "prove" "protect" "presence" "mistreated" "lookout" "intentionally" "honest" "guard" "controlled" "betrays" "belief" "appear" "after" "abused" "mistrust" "abuse" "tests")  t) "[A-Za-z]*\\>" "\\|" "\\<" (regexp-opt '()  t))

      subjugation  (concat "\\<" (regexp-opt '("reject" "wishes" "upper" "rights" "retaliate" "respected" "pleasing" "lengths" "instead" "fear" "demanding" "decisions" "consequences" "confrontations" "choices" "choice" "asking" "account")  t) "[A-Za-z]*\\>" "\\|" "\\<" (regexp-opt '("iron")  t) "\\>")

      self-sacrifice  (concat "\\<" (regexp-opt '("happy" "guilty" "busy" "volunteer" "uncomfortable" "taking" "return" "receiving" "present" "minimal" "loved" "giving" "everyone" "ends" "else's" "disappoint" "couldn't" "comfortable" "self")  t) "[A-Za-z]*\\>" "\\|" "\\<" (regexp-opt '()  t) "\\>")

      social-undesirability  (concat "\\<" (regexp-opt '("wouldn't" "ugly" "status" "socially" "sexually" "self-conscious" "longer" "level" "fat" "education" "dull" "decent" "career" "boring" "attractive" "associate" "undesirab")  t) "[A-Za-z]*\\>")

      unrelenting-standards  (concat "\\<" (regexp-opt '("best" "pressure" "suffering" "suffer" "strive" "standards" "settle" "second" "relax" "push" "pleasure" "performance" "perfect" "order" "off" "mistakes" "mistake" "look" "hook" "health" "happiness" "excuses" "done" "deserve" "criticism" "constant" "competitive" "all" "achieve" "accomplish" "accept" "unrelenting" "number")  t) "[A-Za-z]*\\>"  "\\|" "\\<" (regexp-opt '("under")  t) "\\>")

      vulnerability  (concat "\\<" (regexp-opt '("might" "world" "serious" "attack" "vagrant" "strike" "street" "sick" "precautions" "physician" "heart" "happening" "financial" "escape" "disaster" "diagnosed" "developing" "destitute" "dangerous" "criminal" "crazy" "concerned" "careful" "being" "becoming" "attacked" "anxiety" "anxious" "vulnerability" "natural")  t) "[A-Za-z]*\\>")

      social-isolation  (concat "\\<" (regexp-opt '("was" "tomorrow" "outside" "notice" "loner" "isolated" "fit" "families" "disappeared" "belong" "alienated" "alien")  t) "[A-Za-z]*\\>")

      defectiveness-shame  (concat "\\<" (regexp-opt '("defects" "respect" "real" "man" "woman" "she" "he" "desire" "basic" "worthwhile" "unworthy" "unacceptable" "stay" "secrets" "saw" "reveal" "lovable" "knew" "inner" "inherent" "greatest" "fooling" "flawed" "fears" "fault" "face" "exposed" "drawn" "defective" "critical" "anyone")  t) "[A-Za-z]*\\>"  "\\|" "\\<" (regexp-opt '()  t) "\\>")

      failure  (concat "\\<" (regexp-opt '("failure" "school" "capable" "compare" "embarrassed" "failures" "humiliated" "inadequacies" "incompetent" "intelligent" "sphere" "successful" "talented")  t) "[A-Za-z]*\\>"  "\\|" "\\<" (regexp-opt '()  t) "\\>")

      dependence-incompetence  (concat "\\<" (regexp-opt '("everyday" "responsibilities" "need" "wrong" "unless" "tackling" "solve" "screw" "rely" "relied" "practical" "overwhelming" "issues" "inept" "help" "handling" "guide" "functioning" "dependent" "cope" "confident" "child" "believe" "adult" "ability" "dependence" "incompetence") t) "[A-Za-z]*\\>" "\\|" "\\<" (regexp-opt '()  t) "\\>")
